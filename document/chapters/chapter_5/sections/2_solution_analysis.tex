...
\begin{itemize}
    ...
    A service (computation, storage, etc...) offered by the \textit{Grid} which is requested by a \textit{Customer} through an \textit{Invoking Endpoint}, paying the corresponding \textit{Fee}. In order to be performed, the \textit{Cloud services} coordinate the \textit{Contribution} offered by the \textit{Nodes} offered by \textit{Contributors}. One particular concretization of a Grid Service is the \textit{MapReduce service}.
